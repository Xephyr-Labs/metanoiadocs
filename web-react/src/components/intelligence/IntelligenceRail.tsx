...
export function IntelligenceRail({ data, loading }: { data: Intelligence | null; loading: boolean }) {
  ...
  const Section = ({ icon: Icon, label, count, children }: {
  ...
  const Row = ({ id, title, onCopy }: { id: string; title: string; onCopy?: boolean }) => (
    <div className="group flex items-center gap-1">
      <button onClick={() => ws.select(id)} className="truncate text-left hover:text-accent">{title}</button>
      {onCopy && (
  ...
  return (
    <aside className="flex w-64 shrink-0 flex-col overflow-y-auto border-l border-line bg-canvas">
      <div className="flex items-center gap-1.5 border-b border-line px-3 py-2 text-xs font-semibold">
        ...
        <button onClick={toggle} className="ml-auto text-muted hover:text-ink" title="Hide">
